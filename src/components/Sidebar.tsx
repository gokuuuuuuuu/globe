...
const MOCK_AIRPORTS: AirportData[] = AIRPORTS.map((airport) => {
  const { riskValue, riskZone } = calculateRiskFromEnvironmentRisk(
...
export function Sidebar() {
    ...
    if (type === "from") {
      if (airportCodeFormat === "four") {
        ...
        return (
          ...
          airportCodeMap[flight.fromAirportCode3 || flight.fromAirport] ||
          flight.fromAirportCode3 ||
          flight.fromAirport ||
      ...
      if (airportCodeFormat === "four") {
        // 优先使用航班数据中的四字码，如果没有则从airportCodeMap查找，最后回退到三字码
        return (
          flight.toAirportCode4 ||
          airportCodeMap[flight.toAirportCode3 || flight.toAirport] ||
          flight.toAirportCode3 ||
          flight.toAirport ||
          flight.toAirportZh ||
          "--"
        );
  ...
  const selectedAirport = useMemo(() => {
    ...
    return (
      MOCK_AIRPORTS.find((a) => a.id === selectedAirportForAirline) || null
    );
  }, [selectedAirportForAirline]);

  // 处理航线 View 按钮点击（只触发镜头跟随，不进入下一级）
  const handleFlightViewButtonClick = (
    flight: Flight,
    e?: React.MouseEvent,
  ) => {
    if (e) {
  ...
  useEffect(() => {
    ...
    if (flight) {
      ...
      if (
        ...
        enrichFlightWithAirportCodes(flight)
          .then((enrichedFlight) => {
  ...
  const filteredFlights = useMemo(() => {
    const filtered = MOCK_FLIGHTS.filter((flight) => {
      ...
      if (!searchQuery) {
        // 如果没有搜索查询，只检查状态
        return (
          flightStatuses.length > 0 && flightStatuses.includes(flight.status)
        );
      }
      ...
      const matchesSearch =
        ...
        (flight.fromAirportCode3 &&
          flight.fromAirportCode3.toLowerCase().includes(query)) ||
        (flight.fromAirportCode4 &&
          flight.fromAirportCode4.toLowerCase().includes(query)) ||
        (flight.toAirportCode3 &&
          flight.toAirportCode3.toLowerCase().includes(query)) ||
        (flight.toAirportCode4 &&
          flight.toAirportCode4.toLowerCase().includes(query));
  ...
  useEffect(() => {
    if (
      teamToScrollRef.current &&
      expandedTeamIds.includes(teamToScrollRef.current) &&
      sidebarTab === "person"
    ) {
      // 使用 setTimeout 确保 DOM 已经更新
      setTimeout(() => {
        const teamElement = document.querySelector(
          `[data-team-id="${teamToScrollRef.current}"]`,
        );
        if (teamElement) {
  ...
  useEffect(() => {
    ...
    if (sidebarTab === "person" && searchQuery) {
      ...
      // 检查哪些匹配的机队还没有展开
      const needToExpand = matchingTeamIds.filter(
        (id) => !currentExpanded.includes(id),
      );
  ...
  const getRiskBackgroundColor = (riskZone: RiskZone) => {
    switch (riskZone) {
      case "red":
        return "rgba(255, 23, 68, 0.12)"; // #ff1744 半透明，符合项目深色风格
      case "yellow":
        return "rgba(255, 193, 7, 0.12)"; // #ffc107 半透明
      case "green":
        return "rgba(76, 175, 80, 0.12)"; // #4caf50 半透明
      default:
        return "rgba(30, 41, 59, 0.6)"; // 默认背景色
    }
  };
  ...
  const getRiskHoverBackgroundColor = (riskZone: RiskZone) => {
    switch (riskZone) {
      case "red":
        return "rgba(255, 23, 68, 0.2)"; // hover时更明显
      case "yellow":
        return "rgba(255, 193, 7, 0.2)";
      case "green":
        return "rgba(76, 175, 80, 0.2)";
      default:
        return "rgba(30, 41, 59, 0.8)"; // 默认hover背景色
    }
  };
  ...
  return (
    <div className="sidebar">
      ...
      {/* Tabs和内容区域 - 可收起 */}
      <div
        className={`sidebar-content-wrapper ${isContentCollapsed ? "collapsed" : ""}`}
      >
        {/* Tabs */}
        <div className="sidebar-tabs">
          ...
          >
            ...
            <img
              src={rightBoxActiveIcon}
              alt="box active"
              className="tab-box-bg-active"
            />
        ...
        <div className="sidebar-search">
          <div className="filter-input-wrapper">
            <img
              src={iconInputHighlights}
              alt="highlights"
              className="filter-input-highlights"
            />
            <input
              type="text"
        ...
        {/* 数据提示（仅airline tab显示） */}
        {sidebarTab === "airline" && (
          <div className="sidebar-data-hint">数据仅显示未来24小时内航班</div>
        )}
        ...
        {sidebarTab === "airline" && selectedAirport && (
          <div className="sidebar-selected-airport">
            <div className="selected-airport-header">
              ...
              <button
                className="deselect-airport-btn"
                onClick={handleDeselectAirport}
              >
          ...
          {sidebarTab === "airport" && (
            <div className="sidebar-list">
              ...
              {filteredAirports.map((airport) => {
                ...
                const riskHoverBgColor = getRiskHoverBackgroundColor(
                  airport.riskZone,
                );
                const riskBorderColor =
                  getRiskValueColor(airport.riskZone) + "40"; // 25%透明度的边框
                  ...
                  >
                    <div className="item-left">
                      ...
                      <div className="item-info">
                        <span className="item-info-item">
                          执飞单位: {airport.operatorCount}
                        </span>
                        <span className="item-info-item">
                          航班数: {airport.flightCount}
                        </span>
                      </div>
                    </div>
                ...
                <div className="flight-detail-content">
                  ...
                  {(() => {
                    ...
                    const maxRisk = Math.max(
                      selectedFlight.humanRisk,
                      selectedFlight.machineRisk,
                      selectedFlight.environmentRisk,
                    );
                    const riskBgColor = getFlightRiskBackgroundColor(maxRisk);
                    const riskHoverBgColor =
                      getFlightRiskHoverBackgroundColor(maxRisk);
                    const riskBorderColor =
                      getRiskValueColorFromNumber(maxRisk) + "40"; // 25%透明度的边框
                      ...
                      >
                        ...
                        <div className="flight-item-body">
                          <div className="flight-departure">
                            <div className="flight-time-info">
                              <span className="flight-time-label">
                                预飞 {selectedFlight.scheduledDeparture}
                              </span>
                              <span className="flight-time-value">
                                计飞 {selectedFlight.estimatedDeparture}
                              </span>
                            </div>
                          </div>
                          <div className="flight-status-section">
                            <div className="flight-route">
                              ...
                              <div className="flight-route-airports">
                                <span className="flight-route-from">
                                  {getAirportCode(selectedFlight, "from")}
                                </span>
                                <span className="flight-route-to">
                          ...
                          <div className="flight-arrival">
                            <div className="flight-time-info">
                              <span className="flight-time-label">
                                {selectedFlight.scheduledArrival} 预到
                              </span>
                              <span className="flight-time-value">
                                {selectedFlight.estimatedArrival} 计到
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="flight-item-footer">
                          <div className="flight-risk-values">
                            <span className="flight-risk-value">
                              ...
                              <span
                                style={{
                                  color: getRiskValueColorFromNumber(
                                    selectedFlight.humanRisk,
                                  ),
                                }}
                              >
                                {getRiskLevelText(selectedFlight.humanRisk)}
                              </span>
                            </span>
                            <span className="flight-risk-value">
                              机
                              ...
                              >
                                {getRiskLevelText(selectedFlight.machineRisk)}
                              </span>
                            </span>
                            <span className="flight-risk-value">
                              环
                  ...
                  <div className="detail-basic-info-wrapper">
                    ...
                    <div className="detail-basic-info-box">
                      ...
                      <div className="detail-basic-info-part">
                        ...
                        <div className="detail-info-row">
                          {(() => {
                            ...
                            const pfMember =
                              selectedFlight.crewMembers?.find(
                                (m) => m.role === "Tb",
                              ) || selectedFlight.crewMembers?.[0];
                            const pfPerson = pfMember
                              ? getPersonById(pfMember.personId)
                              : null;

                            // 优先使用person数据中的信息，如果没有则使用航班数据中的
                            const pfId = pfPerson?.pfId || selectedFlight.pfId;
                            const pfTechnology =
                              pfPerson?.pfTechnology ||
                              selectedFlight.pfTechnology;
                      ...
                      {/* 中部分 - 机组成员 */}
                      <div className="detail-basic-info-part">
                        {selectedFlight.crewMembers &&
                          selectedFlight.crewMembers.length > 0 && (
                            <div className="detail-info-row detail-crew-row">
                              <span className="detail-info-label detail-crew-title">
                                机组成员
                              </span>
                              <div className="detail-crew-list">
                                {selectedFlight.crewMembers.map(
                                  (member, idx) => {
                                    // 从person数据中获取完整的人员信息
                                    ...
                                    return (
                                      ...
                                      >
                                        <span className="detail-crew-name">
                                          {person.name}
                                        </span>
                                        ...
                                        <span className="detail-crew-role">
                                          {member.role}
                                        </span>
                                        <span className="detail-crew-id">
                                          {person.pfId}
                                        </span>
                                      </div>
                                    );
                        ...
                        {metarReport && (
                          <div className="detail-info-row">
                            <div className="detail-info-item detail-info-item-full">
                              ...
                              <span className="detail-info-value detail-info-value-multiline">
                                {metarReport}
                              </span>
                            </div>
                          </div>
                ...
                <div className="sidebar-list">
                  {filteredFlights.length === 0 ? (
                    ...
                    filteredFlights.map((flight) => {
                      ...
                      const isHighlighted =
                        highlightedFlightRouteId === flight.id;
                        ...
                        >
                          ...
                          <div className="flight-item-body">
                            <div className="flight-departure">
                              <div className="flight-time-info">
                                <span className="flight-time-label">
                                  预飞 {flight.scheduledDeparture}
                                </span>
                                <span className="flight-time-value">
                                  计飞 {flight.estimatedDeparture}
                                </span>
                              </div>
                            </div>
                            <div className="flight-status-section">
                            ...
                            <div className="flight-arrival">
                              <div className="flight-time-info">
                                <span className="flight-time-label">
                                  {flight.scheduledArrival} 预到
                                </span>
                                <span className="flight-time-value">
                                  {flight.estimatedArrival} 计到
                                </span>
                              </div>
                            </div>
                          </div>
                          <div className="flight-item-footer">
                            <div className="flight-risk-values">
                              <span className="flight-risk-value">
                                ...
                                >
                                  {getRiskLevelText(flight.humanRisk)}
                                </span>
                              </span>
                              <span className="flight-risk-value">
                                机
                                ...
                                >
                                  {getRiskLevelText(flight.machineRisk)}
                                </span>
                              </span>
                              <span className="flight-risk-value">
                                环
                                ...
                                >
                                  {flight.riskLevel ||
                                    getRiskLevelText(flight.environmentRisk)}
                                </span>
                              </span>
                            </div>
                            <button
                              className="flight-view-button"
                              onClick={(e) =>
                                handleFlightViewButtonClick(flight, e)
                              }
                            >
                              <img
                                src={viewIcon}
              ...
              {selectedPersonId ? (
                ...
                <div className="person-detail-content">
                  ...
                  {(() => {
                    ...
                    if (!person)
                      return <div className="sidebar-empty">人员不存在</div>;
                        ...
                        <div className="person-detail-card">
                          ...
                          <div className="person-detail-header">
                            ...
                            <div className="person-detail-header-info">
                              <div className="person-detail-header-name">
                                {person.name}
                              </div>
                              <div className="person-detail-header-meta">
                                <span className="person-detail-meta-label">
                                  PF工号:
                                </span>
                        ...
                        <div className="person-basic-info">
                          <div className="person-basic-title">
                            <img
                              src={radarIcon}
                              alt="radar"
                              className="person-basic-title-icon"
                            />
                            <span>基本信息</span>
                          </div>
                          <div className="person-basic-card">
                            ...
                            <div className="person-basic-item">
                              <span className="person-basic-label">
                                飞行年限:
                              </span>
                              <span className="person-basic-value">
                                {person.flightYears !== undefined
                                  ? `${person.flightYears}年`
                                  : "--"}
                              </span>
                            </div>
                            <div className="person-basic-item">
                              ...
                              <span className="person-basic-value">
                                {person.totalFlightHours !== undefined
                                  ? `${person.totalFlightHours.toLocaleString()}小时`
                                  : "--"}
                              </span>
                            </div>
                            <div className="person-basic-item">
                              ...
                              <span className="person-basic-value">
                                {person.recent90DaysFlightHours !== undefined
                                  ? `${person.recent90DaysFlightHours}小时`
                                  : "--"}
                              </span>
                            </div>
                            <div className="person-basic-item">
                              ...
                              <span className="person-basic-value">
                                {person.certifiedAircraftTypes &&
                                person.certifiedAircraftTypes.length > 0
                    ...
                    filteredTeams.map((team) => {
                      ...
                      return (
                        <div
                          ...
                          style={
                            isExpanded
                              ? { backgroundImage: `url(${personCardBg})` }
                              : {}
                          }
                        >
                          <div
                              ...
                              >
                                <div className="person-row-avatar">
                                  <img
                                    src={personRowIcon}
                                    alt="person"
                                    className="person-row-avatar-img"
                                  />
                                </div>
                                <div className="person-row-main">
                                  <div className="person-row-name">
                                    <span className="person-row-name-text">
                                      {team.leader.name}
                                    </span>
                                    {team.leader.riskValue !== undefined && (
                                      <span
                                  ...
                                  <div className="person-row-meta person-row-meta-column">
                                    <div className="person-row-meta-line">
                                      <span className="person-row-meta-label">
                                        PF技术等级:
                                      </span>
                                      <span className="person-row-meta-value">
                                        {team.leader.pfTechnology}
                                      </span>
                                    </div>
                                    <div className="person-row-meta-line">
                                      <span className="person-row-meta-label">
